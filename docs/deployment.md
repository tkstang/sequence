...
The production API service is connected to the GitHub repository. Commits
pushed to `main` deploy the API when the committed files match the
...
The production web project is connected to the GitHub repository. Commits
pushed to `main` create the production deployment; other branches create preview
deployments.
...
1. The provider deployment metadata points at `tkstang/sequence` on `main` and
   the expected commit SHA.
2. `GET /health` on the API returns `200` and `{"status":"ok"}`.
3. A WebSocket connection can open to the API `/trpc` endpoint.
4. Signup/login sets a session cookie and `health.me` succeeds.
5. Guest invite join sets the guest cookie.
6. Local pass-and-play can create a game and render the board.
7. Two browser contexts can share a remote game and receive move broadcasts.
8. Concede reaches game over in all connected contexts.
9. A 375px viewport shows the board, hand, and player rail without horizontal
   scrolling.
10. A forged-XFF invite preview/join burst cannot bypass the anonymous limiter.
