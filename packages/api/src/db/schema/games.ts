 ...
 *
 * `created_by` references Better Auth's `user.id` at the application layer: the
 * `user` table is owned and generated by Better Auth (p03-t04), so `created_by`
 * is stored as the matching text id without a cross-migration DB-level FK here.
 * `rematch_of`, by contrast, references this same api-owned table, so it carries
 * a real self-FK (`ON DELETE SET NULL`): when a referenced game is deleted
 * (e.g. the p04-t12 expiry sweep) the pointer is nulled rather than left
 * dangling.
 */
...
export const games = pgTable(
  ...
  (table) => [
    ...
    // Self-FK: a rematch points at the game it was spawned from. SET NULL so
    // deleting the source game (p04-t12 expiry sweep) nulls the pointer rather
    // than leaving it dangling or blocking the delete.
    foreignKey({
      columns: [table.rematchOf],
      foreignColumns: [table.id],
      name: 'games_rematch_of_games_id_fk',
    }).onDelete('set null'),
  ],
);
