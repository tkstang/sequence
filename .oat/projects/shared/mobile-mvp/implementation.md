...
| p12-t08 / p01-p12/m2 | design.md           | API surface artifacts documented native auth/config support, `game.join` guest-token return, and presence-correctness fixes                                                        | p12-t08 accepted an additive read-only `game.access(gameId)` route using `gamePlayerProcedure` to confirm a game-scoped credential before destructive native guest cleanup                                                               | The re-review found the shipped route defensible and tested, but missing from lifecycle API-surface artifacts                                                  | `packages/api/src/game/game.router.ts`; `packages/api/src/game/routes/access.ts`; `packages/api/src/game/routes/access.test.ts`; `.oat/projects/shared/mobile-mvp/design.md`                                                                                                                                                                         | p12-t17                                                                                              |
